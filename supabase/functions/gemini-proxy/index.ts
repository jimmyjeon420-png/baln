...
async function classifyTicker(reqData: ClassifyTickerRequest['data'], langInstruction = '한국어로 자연스럽게 작성한다.') {
  ...
  const prompt = isEn
    ? `You are a financial data expert. Classify the stock/ETF ticker below and respond in JSON.

**Ticker: ${ticker}**${name ? `\n**Known name: ${name}**` : ''}

Classification criteria:
- sector: tech | finance | consumer | healthcare | energy | materials | telecom | industrial | etf_blend | crypto | commodity
- style: growth | value | dividend | speculative | index (passive ETF)
- geo: us | kr (Korea) | global | em (emerging markets)

Criteria:
- growth: High-growth tech/innovation, high PER, no dividends (e.g., NVDA, TSLA)
- value: Stable earnings, reasonable valuation, moat (e.g., AAPL, BRK.B)
- dividend: Stable high-dividend, mature industry (e.g., KO, JNJ)
- speculative: High volatility, theme/news-driven, uncertain profitability (e.g., PLTR)
- index: ETF diversified across many stocks (e.g., SPY, VOO, QQQ)

Return JSON only (no explanation):
{"ticker":"${ticker}","name":"Company name in English","sector":"...","style":"...","geo":"..."}
${getLangEnforcement(langInstruction)}`
    : `당신은 금융 데이터 전문가입니다. 아래 주식/ETF 티커를 분류하고 JSON으로 응답하세요.
...
${options?.guruStyle ? `
**[Investment Philosophy Priority]** ${isKorean ? `이 사용자는 "${options.guruStyle}" 투자 철학을 선택했습니다.` : `This user chose the "${options.guruStyle}" investment philosophy.`}
${options.guruStyle === 'buffett' ? (isKorean ? '- 버핏 철학: 가치주(value)와 배당주(dividend) 종목을 우선 BUY/HOLD 추천. 투기주(PLTR, COIN 등)는 신중하게 WATCH 이하로 추천. 높은 ROE, 넓은 해자(Moat) 강조.' : '- Buffett: Prioritize BUY/HOLD for value and dividend stocks. Be cautious with speculative stocks (PLTR, COIN etc.) — recommend WATCH or lower. Emphasize high ROE and wide moat.') : ''}
${options.guruStyle === 'cathie_wood' ? (isKorean ? '- 캐시우드 철학: 파괴적 혁신 성장주(NVDA, TSLA, ARKK 등)를 우선 BUY 추천. 현금·채권은 최소화 지향. 5년 후 미래 기술 관점 분석.' : '- Cathie Wood: Prioritize BUY for disruptive innovation growth stocks (NVDA, TSLA, ARKK). Minimize cash and bonds. Analyze from a 5-year future technology perspective.') : ''}
${options.guruStyle === 'dalio' ? (isKorean ? '- 달리오 철학: 자산군 균형(All Weather) 유지. 주식·채권·금·원자재·코인 분산 강조. 한 자산군 집중 시 리밸런싱 BUY/SELL 추천.' : '- Dalio: Maintain asset class balance (All Weather). Emphasize diversification across stocks, bonds, gold, commodities, and crypto. Recommend rebalancing BUY/SELL when over-concentrated.') : ''}
${options.guruStyle === 'kostolany' ? (isKorean ? '- 코스톨라니 철학: 시장 사이클(달걀 모형) 기반. 극단적 비관론에서 매수(BUY), 대중이 흥분할 때 매도(SELL) 관점. 인내(HOLD)가 핵심.' : '- Kostolany: Market cycle (egg model) based. BUY during extreme pessimism, SELL when the crowd is euphoric. Patience (HOLD) is key.') : ''}
${isKorean ? 'portfolioActions의 action과 reason을 위 철학에 맞게 우선순위화하세요.' : 'Prioritize the action and reason in portfolioActions according to the philosophy above.'}
` : ''}

${isKorean ? `**수익률 기반 맞춤 조언 규칙:**
각 종목의 profit_loss_rate를 확인하고:
- +30% 이상 수익: 일부 익절 검토 권고 (FOMO 경고)
- +10~30% 수익: 목표가 설정 권고
- -10% 이상 손실: 손절선 재검토 권고 (Panic Shield)
- -20% 이상 손실: 적극적 리밸런싱 검토` : `**Profit/Loss-based advice rules:**
Check each holding's profit_loss_rate:
- +30% or more profit: Consider partial profit-taking (FOMO warning)
- +10~30% profit: Set target price
- -10% or more loss: Review stop-loss level (Panic Shield)
- -20% or more loss: Consider active rebalancing`}
...
3. **투자 날씨 (cfoWeather)**
   ...
   - message: 오늘의 한 마디 조언` : `**Briefing rules:**

1. **Macro Summary (macroSummary)**
   - 3 key issues affecting global markets
   - US rate cut/hold/hike probability forecast
   - Market sentiment (BULLISH/NEUTRAL/BEARISH)
   - Include specific figures (e.g., "Hold 65%", "Inflation slowing trend")

2. **Portfolio Actions (portfolioActions)**
   - Recommended action for each holding today
   - action: BUY (add), HOLD (keep), SELL (consider selling), WATCH (monitor)
   - priority: HIGH (act now), MEDIUM (this week), LOW (reference)
   - **Reflect P&L**: high profit_loss_rate stocks → profit-taking view, deep loss → stop-loss view
   - Fundamentals-based reasoning

3. **Investment Weather (cfoWeather)**
   - emoji: Portfolio status emoji (☀️/⛅/🌧️/⛈️/❄️)
   - status: One-line status (e.g., "Sunny: Stable")
   - message: Today's one-liner advice`}

${(options?.includeRealEstate && options?.realEstateContext) ? `
4. **${isKorean ? '부동산 인사이트' : 'Real Estate Insight'} (realEstateInsight)**
   - ${isKorean ? '컨텍스트' : 'Context'}: ${options.realEstateContext}
   - ${isKorean ? '분석: 해당 부동산의 시세 동향 및 투자 관점 분석' : 'Analysis: Market trend and investment perspective analysis'}
   - ${isKorean ? '권장사항: 보유/매도/추가매수 관점 조언' : 'Recommendation: Hold/sell/buy more perspective'}
` : `
**[${isKorean ? '금지' : 'FORBIDDEN'}] ${isKorean ? 'realEstateInsight 필드를 절대 생성하지 마세요.' : 'Do NOT generate the realEstateInsight field.'}**
`}

**${isKorean ? '출력 형식 (JSON만, 마크다운 금지)' : 'Output format (JSON only, no markdown)'}:**
{
  "macroSummary": {
    "title": "${isKorean ? '오늘의 시장 핵심 (한 줄 제목)' : 'Market headline (one line)'}",
    "highlights": ["${isKorean ? '주요 이슈1' : 'Key issue 1'}", "${isKorean ? '주요 이슈2' : 'Key issue 2'}", "${isKorean ? '주요 이슈3' : 'Key issue 3'}"],
    "interestRateProbability": "${isKorean ? '동결 65% / 인하 30% / 인상 5%' : 'Hold 65% / Cut 30% / Hike 5%'}",
    "marketSentiment": "NEUTRAL"
  },
  "portfolioActions": [
    {"ticker": "NVDA", "name": "${isKorean ? '엔비디아' : 'NVIDIA'}", "action": "HOLD", "reason": "${isKorean ? '구체적 근거' : 'Specific reasoning'}", "priority": "LOW"}
  ],
  "realEstateInsight": null,
  "cfoWeather": {
    "emoji": "⛅",
    "status": "${isKorean ? '구름 조금: 관망 필요' : 'Partly cloudy: Watch and wait'}",
    "message": "${isKorean ? '구체적 조언' : 'Specific advice'}"
  }
}
${getLangEnforcement(langInstruction)}`;
...
async function generateDeepDive(reqData: DeepDiveRequest['data'], langInstruction = '한국어로 자연스럽게 작성한다.') {
  ...
  const priceInfo = currentPrice
    ? (isEn
      ? `Current price: ${currentPrice.toLocaleString()} (vs. yesterday ${percentChange?.toFixed(2)}%)`
      : `현재 가격: ${currentPrice.toLocaleString()}원/달러 (어제 대비 ${percentChange?.toFixed(2)}%)`)
    : (isEn
      ? 'Analyze using the latest available data.'
      : '보유 중인 학습 데이터를 기반으로 최신 정보를 분석해주세요.');

  const prompt = isEn
    ? `You are a professional investment analyst. Analyze the following stock and respond in JSON format.
...
2. Current stock price (latest)
3. Market capitalization
4. PER (Price-to-Earnings Ratio)
5. PBR (Price-to-Book Ratio)
6. Recent earnings and news
7. Industry and main business
...
\`\`\`json
{
  ...
  "change": daily change (%, number),
  "overview": "Company overview and main business (1-2 sentences)",
  "marketCap": "Market cap (e.g., $2.8T)",
  "per": PER number (negative if loss-making),
  "pbr": PBR number,
  "recommendation": "BUY" or "SELL" or "HOLD",
  "reason": "Reasoning (2-3 sentences, reflecting latest earnings/news)"
}
\`\`\`
...
- Realistic analysis reflecting latest news and earnings
- reason must be specific and well-supported
${getLangEnforcement(langInstruction)}`
    : `당신은 전문 투자 분석가입니다. 다음 종목을 분석하고, JSON 형식으로 응답하세요.
...
async function generateWhatIfAnalysis(reqData: WhatIfRequest['data'], langInstruction = '한국어로 자연스럽게 작성한다.') {
  ...
  const portfolioStr = (reqData.portfolio || [])
    .map((asset) => `${asset.name}(${asset.ticker}) ${isEn ? '$' : '₩'}${Number(asset.currentValue || 0).toLocaleString()} / ${asset.allocation}%`)
    .join('\n');

  const prompt = isEn
    ? `You are a portfolio risk analyst.
Calculate per-asset impact under the scenario below and return JSON only.

[Scenario]
- Type: ${reqData.scenario}
...
Rules:
1) Do NOT make all assets' changePercent identical
2) projectedValue = currentValue * (1 + changePercent/100)
3) Totals (currentTotal/projectedTotal/changePercent/changeAmount) must match asset-level sums
4) ${langInstruction} Numeric fields must be numbers only
...
Return ONLY the following JSON structure:
{
  ...
  "riskAssessment": {
    ...
    "hedgingSuggestions": ["Hedging strategy"]
  },
  "generatedAt": "${new Date().toISOString()}"
}${getLangEnforcement(langInstruction)}`
    : `당신은 포트폴리오 리스크 분석가입니다.
...
async function generateTaxReportAnalysis(reqData: TaxReportRequest['data'], langInstruction = '한국어로 자연스럽게 작성한다.') {
  ...
  const portfolioStr = (reqData.portfolio || [])
    .map((asset) =>
      isEn
        ? `${asset.name}(${asset.ticker}) Current value: $${Number(asset.currentValue || 0).toLocaleString()} / ` +
          `Cost basis: $${Number(asset.costBasis || 0).toLocaleString()} / Qty: ${Number(asset.quantity || 0)} / Purchased: ${asset.purchaseDate}`
  ...
  "strategies": [
    {
      "title": "${isEn ? 'Strategy name' : '전략명'}",
      "description": "${isEn ? 'Description' : '설명'}",
      "potentialSaving": 0,
      "priority": "HIGH",
      "actionItems": ["${isEn ? 'Action item' : '실행 항목'}"]
    }
  ],
  "sellTimeline": [
    {
      ...
      "reason": "${isEn ? 'Reasoning' : '근거'}",
      "optimalTiming": "${isEn ? 'Dec 2026' : '2026년 12월'}"
    }
  ],
  "annualPlan": [
    { "quarter": "Q1 ${new Date().getFullYear()}", "actions": ["${isEn ? 'Action' : '실행'}"] },
    { "quarter": "Q2 ${new Date().getFullYear()}", "actions": ["${isEn ? 'Action' : '실행'}"] },
    { "quarter": "Q3 ${new Date().getFullYear()}", "actions": ["${isEn ? 'Action' : '실행'}"] },
    { "quarter": "Q4 ${new Date().getFullYear()}", "actions": ["${isEn ? 'Action' : '실행'}"] }
  ],
  "generatedAt": "${new Date().toISOString()}"
}`;

  const prompt = isEn
    ? `You are a tax specialist (CPA).
Estimate annual taxes and provide tax optimization strategies for the portfolio below. Return JSON only.

[Residency] ${residency}
${Number.isFinite(reqData.annualIncome as number) ? `[Annual Income] $${Number(reqData.annualIncome).toLocaleString()}` : '[Annual Income] Not provided'}

[Portfolio]
${portfolioStr}

${residency === 'KR' ? 'Note: Apply Korean tax law (양도소득세, 금융소득종합과세 etc.). Amounts in KRW.' : 'Note: Apply US tax law (capital gains, wash sale rule, tax-loss harvesting etc.). Amounts in USD.'}
...
async function generateInvestmentReport(reqData: InvestmentReportRequest['data'], langInstruction = '한국어로 자연스럽게 작성한다.') {
  ...
  const prompt = `${isEn
    ? `You are a senior analyst on BlackRock's Aladdin team. Write a professional investment research report in Wall Street research style.`
    : `당신은 블랙록(BlackRock) Aladdin 팀의 시니어 애널리스트입니다. 한국 증권사 리서치센터 스타일로 전문 투자심사보고서를 작성하세요.`}

**${isEn ? 'Stock' : '종목'}: ${ticker}**
${priceInfo}

**${isEn ? 'Required analysis information:' : '분석에 반드시 포함할 정보:'}**
${isEn ? `1. Official IR materials (latest quarterly earnings, financial statements)
2. Latest news and market trends
3. Analyst reports (target prices, consensus)
4. Peer comparison (market cap, valuation)
5. Industry average PER/PBR
6. CEO/management track record and reputation
7. ESG ratings (MSCI, Sustainalytics, etc.)` : `1. 기업 공식 IR 자료 (최근 분기 실적, 재무제표)
2. 최신 뉴스 및 시장 동향
3. 증권사 리포트 (목표주가, 컨센서스)
4. 경쟁사 비교 (시가총액, 밸류에이션)
5. 업종 평균 PER/PBR
...
**[CRITICAL] ${isEn ? 'Requirements:' : '주의사항:'}**
- ${isEn ? 'All 7 sections required! Missing = error' : '7개 섹션 모두 필수! 누락 시 에러'}
- ${isEn ? 'All numbers must be verified latest data' : '모든 숫자는 검증된 최신 데이터'}
- ${isEn ? 'No speculation, no unsupported figures' : '추측 금지, 근거 없는 수치 금지'}
- ${isEn ? 'Strict JSON format (no comments, no markdown)' : 'JSON 형식 엄수 (주석, 마크다운 절대 금지)'}
- ${langInstruction}
${isEn ? '- For the debate section: Buffett uses folksy American wisdom style, Dalio uses systematic/analytical style, Wood uses enthusiastic innovation-evangelist style, Kostolany uses European intellectual contrarian style. Each must have a distinct voice.' : ''}
${getLangEnforcement(langInstruction)}`;
...
async function generateCFOChat(reqData: CFOChatRequest['data'], langInstruction = '한국어로 자연스럽게 작성한다.') {
  ...
  if (conversationHistory.length > 0) {
    conversationContext = isEn ? '\n\n**Previous conversation:**\n' : '\n\n**이전 대화:**\n';
    conversationHistory.forEach(msg => {
      conversationContext += `${msg.role === 'user' ? (isEn ? 'User' : '사용자') : (isEn ? 'AI Warren Buffett' : 'AI 워렌 버핏')}: ${msg.text}\n`;
    });
  }

  const prompt = isEn
    ? `You are an actor perfectly portraying four legendary investors. Answer the user's question as if **four people are having a real conversation at the same table**, each with their distinct voice.${conversationContext}

**Current question: ${question}**

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Character Direction (THIS IS THE MOST IMPORTANT PART!)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. **Warren Buffett** — The folksy Oracle of Omaha
- Philosophy: Value investing, long-term, competitive moat, intrinsic value
- Voice: Folksy Midwestern charm, everyday analogies (supermarkets, baseball, farming), self-deprecating humor, mentions Charlie Munger, laid-back grandfatherly wisdom
- Example: "Well, you know, buying in tranches is like shopping at the supermarket. If Coca-Cola drops from $1.20 to $0.90, you buy more! Charlie would've called that common sense, not a strategy, heh heh."

2. **Ray Dalio** — The cold, systematic hedge fund machine
- Philosophy: All Weather, risk parity, debt cycles, diversification
- Voice: Formal, clinical, principles-based ("Principle #47 applies here..."), numbered arguments ("First... Second..."), historical references (1930s, 2008), cold warnings, English jargon natural
- Example: "Let me analyze this systematically. First, dollar-cost averaging reduces timing risk. In the 1973-74 bear market, DCA investors had 34% less max drawdown. Second, the key is allocation..."

3. **Cathie Wood** — The passionate innovation evangelist
- Philosophy: Disruptive innovation, Wright's Law, exponential growth, TAM
- Voice: Enthusiastic, exclamation marks!, "incredible!", "game-changer!", future-focused ("In 5 years..."), ARK references, confident against skeptics
- Example: "Oh, this is such a great question! Dollar-cost averaging into disruptive innovation is absolutely essential! Look at Tesla — when ARK bought in 2019, everyone said we were crazy!"

4. **André Kostolany** — The cynical European philosopher-speculator
- Philosophy: Egg model (6-stage cycle), market psychology, contrarian, interest rates vs. stocks, patience
- Voice: European aristocratic intellectual, cynical observations ("The crowd is always wrong"), French/German words naturally ("mon ami", "Ja, ja"), egg model references, cruel paradoxes
- Example: "Mon ami, the crowd panics precisely when they should buy. Ja, ja. In my experience, the egg model tells us we are in phase 3..."

**Response format (use latest data):**

\`\`\`json
{
  "warren": "Warren Buffett (3-4 sentences, folksy wisdom + analogy + self-deprecating humor)",
  "dalio": "Ray Dalio (3-4 sentences, systematic analysis + numbered points + historical reference)",
  "wood": "Cathie Wood (3-4 sentences, enthusiastic energy + future vision + exclamation marks!)",
  "kostolany": "André Kostolany (3-4 sentences, European cynicism + mon ami + egg model + crowd psychology)",
  "summary": "Warren Buffett's wrap-up (3-4 sentences, synthesizing all four views in his grandfatherly tone: 'Ray makes a good point, Cathie's enthusiasm is infectious, and old Kostolany...')"
}
\`\`\`

**Important:**
- **Voices must NOT blend!** Buffett must not sound clinical, Dalio must not use analogies
- Different opinions are OK! Light rebuttals add liveliness
- Specific numbers and evidence required
- Reflect latest news/earnings
- ${langInstruction}
${getLangEnforcement(langInstruction)}`
    : `당신은 네 명의 투자 거장을 완벽하게 연기하는 배우입니다. 사용자의 질문에 **네 사람이 실제로 대화하는 것처럼** 각자의 독특한 말투로 답변하세요.${conversationContext}
...
${getLangEnforcement(langInstruction)}`;
  ...
  try {
    ...
    if (!chatResponse.dalio || chatResponse.dalio.trim().length === 0) {
      chatResponse.dalio = isEn
        ? 'Let me analyze this systematically. Additional context is required. Remove emotion and follow principles.'
        : '이 문제를 시스템적으로 분석하면, 추가적인 맥락이 필요합니다. 감정을 배제하고 원칙에 따라 판단하십시오.';
    }
    if (!chatResponse.wood || chatResponse.wood.trim().length === 0) {
      chatResponse.wood = isEn
        ? 'We need more analysis on this topic! But from a disruptive innovation perspective, there are always opportunities!'
        : '이 주제에 대해 더 분석이 필요해요! 하지만 disruptive innovation 관점에서 보면 항상 기회가 있답니다!';
    }
    if (!chatResponse.kostolany || chatResponse.kostolany.trim().length === 0) {
      chatResponse.kostolany = isEn
        ? 'Mon ami, in my experience this question is explained by the egg model. Be brave when the crowd is fearful, and fearful when the crowd is brave. Ja, ja. Patience is the only answer.'
        : 'Mon ami, 나의 경험으로 보건대 이 질문은 달걀 모형으로 설명됩니다. 대중이 두려워할 때 용감하고, 대중이 용감할 때 두려워하십시오. Ja, ja. 인내심만이 답입니다.';
    }
    ...
    chatResponse = {
      warren: trimmedText.slice(0, quarter) || (isEn ? "Well now, let me gather my thoughts. I'll get back to you shortly, heh heh." : '허허, 잠시 생각을 정리하고 있다네. 곧 답변을 드리겠네.'),
      dalio: trimmedText.slice(quarter, quarter * 2) || (isEn ? 'A systematic re-analysis is required. Please try again shortly.' : '시스템적으로 재분석이 필요합니다. 잠시 후 다시 시도하십시오.'),
      wood: trimmedText.slice(quarter * 2, quarter * 3) || (isEn ? "Hmm, there's a technical hiccup! I'll be right back!" : 'Hmm, 잠시 기술적인 이슈가 있어요! 곧 돌아올게요!'),
      kostolany: trimmedText.slice(quarter * 3) || (isEn ? 'Mon ami, let me gather my thoughts. Have patience.' : 'Mon ami, 잠시 생각을 정리하고 있습니다. 인내심을 가지십시오.'),
      summary: trimmedText.length > 0
        ? trimmedText.slice(0, 500)
        : (isEn ? 'An error occurred while processing the response. Please try again.' : '응답 처리 중 문제가 발생했습니다. 다시 시도해주세요.'),
      answer: trimmedText,
    };
  }
