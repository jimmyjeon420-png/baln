...
export const FACTOR_EXPLANATIONS: Record<FactorType, FactorExplanation> = {
  allocation_drift: {
    title: '배분 이탈도',
    icon: '🎯',
    why: '목표 배분과 실제 배분의 차이를 나타내요. 이탈도가 높으면 원래 계획했던 리스크 수준에서 벗어나 있다는 뜻이에요. 시장이 급변할 때 손실이 커질 수 있어요.',
    example: '2020년 테슬라가 급등했을 때, 포트폴리오의 40%까지 늘어난 투자자들이 많았어요. 그 후 2022년 급락하면서 큰 손실을 봤죠. 목표 배분을 지켰다면 손실을 절반으로 줄일 수 있었어요.',
    solution: '3개월마다 리밸런싱하세요. 너무 많아진 자산은 팔고, 부족한 자산을 사서 원래 목표 비중으로 돌아가세요. 앱의 "처방전" 기능이 자동으로 계산해드려요.',
    historicalContext: '2008년 금융위기 때는 평균 이탈도가 45%였어요. 12% 이하면 안전한 구간입니다.',
  },
