  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.summaryCard}>
        ...
        <Text style={styles.cardTitle}>{t('taxReport.summary')}</Text>
        <Text style={styles.residency}>
          {result.residency === 'KR' ? t('taxReport.residentKR') : t('taxReport.residentUS')}
        </Text>
        ...
        <View style={styles.effectiveRateRow}>
          <Text style={styles.effectiveLabel}>{t('taxReport.effectiveTaxRate')}</Text>
      ...
      <View style={styles.sectionCard}>
        <Text style={styles.sectionTitle}>{t('taxReport.strategies')}</Text>
              ...
              <View style={styles.strategyHeader}>
                ...
                <View style={[styles.priorityBadge, { backgroundColor: priorityColor + '20' }]}>
                  <Text style={[styles.priorityText, { color: priorityColor }]}>
                    {strategy.priority === 'HIGH' ? t('taxReport.priorityHigh') : strategy.priority === 'MEDIUM' ? t('taxReport.priorityMedium') : t('taxReport.priorityLow')}
                  </Text>
                </View>
              </View>
      ...
      {/* 매도 타이밍 */}
      <View style={styles.sectionCard}>
        <Text style={styles.sectionTitle}>{t('taxReport.sellTiming')}</Text>
        {(result.sellTimeline ?? []).map((item, i) => {
          ...
          const actionLabel = {
            SELL_NOW: t('taxReport.actionSellNow'),
            HOLD_FOR_TAX: t('taxReport.actionHoldForTax'),
            TAX_LOSS_HARVEST: t('taxReport.actionTaxLossHarvest'),
          }[item.suggestedAction];
      ...
      {/* 연간 플랜 */}
      <View style={styles.sectionCard}>
        <Text style={styles.sectionTitle}>{t('taxReport.annualPlan')}</Text>
...
const styles = StyleSheet.create({
  ...
  cardTitle: { color: '#FFF', fontSize: 19, fontWeight: '700', marginTop: 8 },
  residency: { color: '#888', fontSize: 14, marginTop: 4 },
  ...
  taxLabel: { color: '#888', fontSize: 13, marginBottom: 4 },
  taxValue: { color: '#FFF', fontSize: 16, fontWeight: '700' },
  totalRow: {
    flexDirection: 'row',
  ...
  totalValue: { color: '#CF6679', fontSize: 19, fontWeight: '800' },
  effectiveRateRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    width: '100%',
  ...
  effectiveLabel: { color: '#888', fontSize: 14 },
  effectiveValue: { color: '#FFA726', fontSize: 15, fontWeight: '700' },
  sectionCard: {
    backgroundColor: '#1E1E1E',
    borderRadius: 16,
    padding: 16,
  ...
  strategyTitle: { color: '#FFF', fontSize: 15, fontWeight: '700', flex: 1 },
  priorityBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  priorityText: { fontSize: 11, fontWeight: '700' },
  strategyDesc: { color: '#AAA', fontSize: 14, lineHeight: 19, marginBottom: 6 },
  savingRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 4 },
  savingText: { color: '#4CAF50', fontSize: 14, fontWeight: '700' },
  actionItem: { color: '#CCC', fontSize: 13, lineHeight: 19 },
  timelineItem: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  timelineLeft: {},
  timelineTicker: { color: '#FFF', fontSize: 15, fontWeight: '700' },
  timelineName: { color: '#666', fontSize: 12 },
  timelineRight: { alignItems: 'flex-end' },
  actionBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  actionLabel: { fontSize: 12, fontWeight: '700' },
  timing: { color: '#888', fontSize: 12, marginTop: 2 },
  ...
  quarterText: { color: '#7C4DFF', fontSize: 13, fontWeight: '700' },
  quarterAction: { color: '#CCC', fontSize: 14, lineHeight: 21 },
  disclaimer: { color: '#555', fontSize: 11, textAlign: 'center', marginTop: 4 },
});
