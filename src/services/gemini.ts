...
[출력 형식] 반드시 아래 JSON 구조로 반환:
{
  ...
  "sections": {
    "financial": {
      ...
      "metrics": [
        ...
        {"label": "매출성장률", "value": "<실제값>% (가속/감속)", "status": "<good|neutral|bad>"},
        {"label": "FCF", "value": "<양수/음수>", "status": "<good|neutral|bad>"},
        {"label": "부채비율", "value": "<실제값>%", "status": "<good|neutral|bad>"},
        {"label": "시가총액", "value": "<약 X조원>", "status": "neutral"}
      ]
    },
    "technical": {
      "title": "기술적 분석",
      "score": <0-100>,
      "highlights": ["기술적 분석 내용 3개 이상"],
      "signals": [
        {"indicator": "RSI", "signal": "<과매수|중립|과매도>", "value": "<실제값>"},
        {"indicator": "MACD", "signal": "<골든크로스|데드크로스|중립>", "value": "<실제값>"},
        {"indicator": "이동평균", "signal": "<정배열|역배열|혼합>", "value": "<수치>"},
        {"indicator": "볼린저밴드", "signal": "<상단|중앙|하단>", "value": "<위치>"},
        {"indicator": "거래량", "signal": "<급증|보통|급감>", "value": "<비율>"}
      ]
    },
    "news": {
      "title": "뉴스 분석",
      "sentiment": "<VERY_POSITIVE|POSITIVE|NEUTRAL|NEGATIVE|VERY_NEGATIVE>",
      "highlights": ["뉴스 기반 분석 3개 이상"],
      "recentNews": [{"title": "<뉴스 제목>", "impact": "<긍정적|중립|부정적>", "date": "<YYYY-MM-DD>"}]
    },
    "quality": {
      "title": "투자 품질",
      "score": <0-100>,
      ...
      "metrics": [
        {"label": "경쟁우위(Moat)", "value": "<강력|보통|약함>", "status": "<good|neutral|bad>", "detail": "<설명>"},
        {"label": "경영진", "value": "<우수|보통|우려>", "status": "<good|neutral|bad>", "detail": "<설명>"},
        {"label": "산업 성장성", "value": "<고성장|성숙|쇠퇴>", "status": "<good|neutral|bad>", "detail": "<전망>"}
      ]
    },
    "aiOpinion": {
      "title": "AI 종합 의견",
  ...
  "quarterDetail": {
    "quarter": "<최근 분기>",
    "revenueSegments": [
      {"name": "<사업부명>", "amount": <금액>, "percentage": <비중%>, "color": "#6366F1", "growth": <성장률%>}
    ],
    "costItems": [
      {"name": "매출원가", "amount": <금액>, "percentage": <비중%>},
      {"name": "판관비", "amount": <금액>, "percentage": <비중%>},
      {"name": "연구개발비", "amount": <금액>, "percentage": <비중%>}
    ],
    "waterfall": [
      {"label": "매출", "amount": <금액>, "type": "revenue"},
      {"label": "매출원가", "amount": <금액>, "type": "cost"},
      {"label": "매출총이익", "amount": <금액>, "type": "subtotal"},
      {"label": "판관비", "amount": <금액>, "type": "cost"},
      {"label": "영업이익", "amount": <금액>, "type": "income"},
      {"label": "기타손익", "amount": <금액>, "type": "cost"},
      {"label": "순이익", "amount": <금액>, "type": "income"}
    ],
    "operatingMargin": <영업이익률(%)>,
    "netMargin": <순이익률(%)>,
    "keyTakeaway": "<분기 핵심 포인트>"
  },
...
■ financial.score (0-100, start at 40):
  ...
  - P/E: Use Forward P/E vs industry avg (Google Search for industry avg)
    → <0.7× avg: Undervalued(+15) / 0.7~1.3×: Fair(+5) / 1.3~2.0×: Overvalued(-5) / >2.0×: Highly overvalued(-10)
    ※ Loss-making: revenue growth 30%+ → neutral(0), else (-15)
  - P/B: Asset-heavy sectors (finance/mfg): <1(+10)/1~2(+5)/2~3(0)/3+(-5)
         Asset-light (SW/SaaS/biotech): <5(+5)/5~15(0)/15~30(-3)/30+(-5)

  [Profitability — max ±25 pts]
  - ROE (3-yr avg preferred): 20%+(+15)/15~20%(+10)/10~15%(+5)/5~10%(0)/<5%(-10)
  - Operating Margin vs industry avg: 1.5×avg(+10)/above avg(+5)/below avg(0)/negative(-10)
  ...
  - Revenue YoY: 30%+ accelerating(+15)/30%+ decelerating(+10)/20~30%(+8)/10~20%(+5)/0~10%(0)
                 Negative cyclical(-5)/Negative structural(-10)

  [Stability — max ±15 pts]
  - Debt/Equity: <50%(+8)/50~100%(+3)/100~200%(0)/>200%(-7)
  - Earnings stability: All 4 qtrs profitable(+5)/mixed(0)/consecutive losses(-5)

■ technical.score (0-100, start at 50):
  - Moving averages (20/60/120-day): Bullish(+15)/Bearish(-15)/Mixed(0)
  - Price vs 120-day MA: >10% above(+5)/above(+3)/below(-3)/>20% below(-5)
  - RSI: 40~60(+5)/60~70(+8)/30~40(-3)/>70 overbought(-8)/<30 contrarian(+10)
  - MACD: Golden cross(+7)/Death cross(-7)/Neutral(0)
  - Volume: >200% avg(+5)/<50%(-5)/normal(0)
  - Bollinger Bands: Near lower(+5)/Breaking upper(-3)/Middle(+2)

■ quality.score (0-100, start at 50):
  - Moat: 2+ types(+20)/1 type(+10)/unclear(0)/low barriers(-10)
  - Management: Excellent(+10)/Average(0)/Concerning(-10)
  - Industry outlook: High-growth(+10)/Mature(0)/Declining(-10)

■ overallScore = financial×0.55 + technical×0.15 + news_score×0.15 + quality×0.15
  news_score: VERY_POSITIVE(90)/POSITIVE(70)/NEUTRAL(50)/NEGATIVE(30)/VERY_NEGATIVE(10)

■ recommendation: 78+: VERY_POSITIVE / 63~77: POSITIVE / 42~62: NEUTRAL / 28~41: NEGATIVE / ≤27: VERY_NEGATIVE

[Required Analysis Sections]
1. Financial Analysis: P/E, P/B, ROE, revenue growth, operating margin, D/E ratio + market cap + last 4 quarters
2. Technical Analysis: RSI, MACD, moving averages (20/60/120-day), Bollinger Bands, volume trend
3. News/Events Analysis: At least 3 recent key news items + sentiment
4. AI Summary Opinion: Buy/sell opinion, price target, bull/bear scenarios
5. Quarterly Earnings: Last 4 quarters (actual reported figures)
6. Latest Quarter Detail: Revenue segments, key costs, waterfall breakdown
7. Valuation: marketCap (KRW number), P/E, P/B (numbers)
8. Data Sources: List of sources used

[Output Format] Return valid JSON only:
{
  "ticker": "${input.ticker}",
  "name": "${input.name}",
  "overallScore": <0-100 calculated>,
  "recommendation": "<VERY_POSITIVE|POSITIVE|NEUTRAL|NEGATIVE|VERY_NEGATIVE>",
  "sections": {
    "financial": {
      "title": "Financial Analysis",
      "score": <0-100>,
      "highlights": ["3+ financial analysis points"],
      "metrics": [
        {"label": "P/E Ratio", "value": "<actual>", "status": "<good|neutral|bad>"},
        {"label": "P/B Ratio", "value": "<actual>", "status": "<good|neutral|bad>"},
        {"label": "ROE", "value": "<actual>%", "status": "<good|neutral|bad>"},
        {"label": "Operating Margin", "value": "<actual>%", "status": "<good|neutral|bad>"},
        {"label": "Revenue Growth", "value": "<actual>% (accelerating/decelerating)", "status": "<good|neutral|bad>"},
        {"label": "FCF", "value": "<positive/negative, ~$XB>", "status": "<good|neutral|bad>"},
        {"label": "Debt/Equity", "value": "<actual>%", "status": "<good|neutral|bad>"},
        {"label": "Market Cap", "value": "<~$XT>", "status": "neutral"}
      ]
    },
    "technical": {
      "title": "Technical Analysis",
      "score": <0-100>,
      "highlights": ["3+ technical analysis points"],
      "signals": [
        {"indicator": "RSI", "signal": "<overbought|neutral|oversold>", "value": "<actual>"},
        {"indicator": "MACD", "signal": "<golden cross|death cross|neutral>", "value": "<actual>"},
        {"indicator": "Moving Averages", "signal": "<bullish|bearish|mixed>", "value": "<values>"},
        {"indicator": "Bollinger Bands", "signal": "<upper|middle|lower>", "value": "<position>"},
        {"indicator": "Volume", "signal": "<surge|normal|decline>", "value": "<vs 20-day avg>"}
      ]
    },
    "news": {
      "title": "News Analysis",
      "sentiment": "<VERY_POSITIVE|POSITIVE|NEUTRAL|NEGATIVE|VERY_NEGATIVE>",
      "highlights": ["3+ news-based points"],
      "recentNews": [{"title": "<actual headline>", "impact": "<positive|neutral|negative>", "date": "<YYYY-MM-DD>"}]
    },
    "quality": {
      "title": "Investment Quality",
      "score": <0-100>,
      "highlights": ["3+ moat/management/industry points"],
      "metrics": [
        {"label": "Competitive Moat", "value": "<strong|moderate|weak>", "status": "<good|neutral|bad>", "detail": "<description>"},
        {"label": "Management", "value": "<excellent|average|concerning>", "status": "<good|neutral|bad>", "detail": "<description>"},
        {"label": "Industry Growth", "value": "<high-growth|mature|declining>", "status": "<good|neutral|bad>", "detail": "<outlook>"}
      ]
    },
    "aiOpinion": {
      "title": "AI Summary Opinion",
      "summary": "<2-3 sentence summary>",
      "bullCase": ["2+ bull scenarios"],
      "bearCase": ["2+ bear scenarios"],
  ...
  "generatedAt": "${new Date().toISOString()}",
  "quarterlyData": [
    {"quarter": "2024 Q1", "revenue": <KRW>, "operatingIncome": <KRW>, "netIncome": <KRW>},
    {"quarter": "2024 Q2", "revenue": <KRW>, "operatingIncome": <KRW>, "netIncome": <KRW>},
    {"quarter": "2024 Q3", "revenue": <KRW>, "operatingIncome": <KRW>, "netIncome": <KRW>},
    {"quarter": "2024 Q4", "revenue": <KRW>, "operatingIncome": <KRW>, "netIncome": <KRW>}
  ],
  "quarterDetail": {
    "quarter": "<most recent quarter>",
    "revenueSegments": [
      {"name": "<segment>", "amount": <KRW>, "percentage": <weight%>, "color": "#6366F1", "growth": <YoY%>}
    ],
    "costItems": [
      {"name": "Cost of Revenue", "amount": <KRW>, "percentage": <%>},
      {"name": "SG&A", "amount": <KRW>, "percentage": <%>},
      {"name": "R&D", "amount": <KRW>, "percentage": <%>}
    ],
    "waterfall": [
      {"label": "Revenue", "amount": <KRW>, "type": "revenue"},
      {"label": "Cost of Revenue", "amount": <KRW>, "type": "cost"},
      {"label": "Gross Profit", "amount": <KRW>, "type": "subtotal"},
      {"label": "SG&A", "amount": <KRW>, "type": "cost"},
      {"label": "Operating Income", "amount": <KRW>, "type": "income"},
      {"label": "Other Inc/Exp", "amount": <KRW>, "type": "cost"},
      {"label": "Net Income", "amount": <KRW>, "type": "income"}
    ],
    "operatingMargin": <% number>,
    "netMargin": <% number>,
    "keyTakeaway": "<one-sentence key takeaway>"
  },
  "marketCap": <KRW number only>,
  "per": <number only>,
  "pbr": <number only>,
  "dataSources": [
    ${hasFundamentals ? '{"name": "Yahoo Finance API", "detail": "Market cap, P/E, P/B, ROE, operating margin, revenue growth, D/E, quarterly earnings' + (isUSD && exchangeRate ? ` (FX rate: ${exchangeRate.toFixed(2)} KRW applied)` : '') + '", "date": "' + (fundamentals?.fetchedAt || new Date().toISOString()) + '"},' : ''}
    {"name": "Google Search", "detail": "Technical analysis, news, segment details", "date": "${new Date().toISOString().split('T')[0]}"}
  ]
}
...
4. recommendation must match overallScore threshold (analyst rating, not investment advice).
5. ${getPromptLanguageInstruction()} ${getFinanceTermGuide()}
6. quarterlyData must be based on actual reported earnings.
7. Assign colors sequentially: "#6366F1", "#8B5CF6", "#EC4899", "#F59E0B", "#10B981", "#3B82F6".
8. Numeric fields (marketCap, per, pbr, revenue, etc.) must be numbers only — no ₩, $, or text suffixes.
9. Include at least 2 dataSources.
10. Market cap check: US mega-caps (TSLA, AAPL, NVDA) must be hundreds of trillions KRW minimum.
11. KRW conversion: ${isUSD && exchangeRate ? `Apply FX rate ${exchangeRate.toFixed(2)} KRW.` : 'Apply real-time FX rate.'} No ₩ in numeric JSON fields.
${hasFundamentals ? '12. Use API-provided data (market cap, P/E, P/B, ROE, etc.) as-is. Do not modify.' : ''}
`);
...
중요: 유효한 JSON만 반환. 금액은 숫자로. 한국어로 자연스럽게 작성한다. 설명 텍스트를 JSON 앞뒤에 붙이지 마세요.
` : `
...
Multiply the base magnitude (${magnitude}%) by each asset class beta. Never apply the same percentage to all assets.

Asset Class Beta Reference (${scenarioLabel} scenario):
${betaGuideStr}

Calculation examples (${scenarioLabel} ${magnitude}% scenario):
- Tech: ${magnitude}% × ${betaTable.high_vol_tech} = ${(magnitude * betaTable.high_vol_tech).toFixed(1)}%
- Defensive: ${magnitude}% × ${betaTable.defensive} = ${(magnitude * betaTable.defensive).toFixed(1)}%
- Gold: ${magnitude}% × ${betaTable.gold} = ${(magnitude * betaTable.gold).toFixed(1)}%${betaTable.gold < 0 ? ' (inverse correlation!)' : ''}
- Crypto: ${magnitude}% × ${betaTable.crypto} = ${(magnitude * betaTable.crypto).toFixed(1)}%

[★ Absolute Rules ★]
1. If all assets have the same changePercent, that is incorrect.
2. Assets with negative beta must move opposite to the scenario direction.
3. Higher beta = larger magnitude of change.
4. projectedValue = currentValue × (1 + changePercent/100), calculated exactly.

[Analysis Items]
1. Overall impact summary (totalImpact) — portfolio weighted-average change
2. Per-asset impact (assetImpacts) — each asset has a different changePercent
3. Risk assessment + hedging strategy (riskAssessment)

[Output Format] Return only the JSON structure below (no explanatory text):
{
  "scenario": "${input.description}",
  "summary": "Overall summary (specify which assets are most vulnerable and which are defensive)",
  "totalImpact": {
    "currentTotal": <portfolio total current value>,
    "projectedTotal": <projected total after scenario>,
    "changePercent": <weighted-average change %>,
    "changeAmount": <value change in KRW>
  },
  "assetImpacts": [
    {
      "ticker": "<ticker>",
      ...
      "changePercent": <individual change % — must differ per asset>,
      "impactLevel": "HIGH/MEDIUM/LOW",
      "explanation": "One sentence explaining why this asset is impacted this much"
    }
  ],
  "riskAssessment": {
    "overallRisk": "HIGH/MEDIUM/LOW",
    "vulnerabilities": ["2-3 vulnerabilities"],
    "hedgingSuggestions": ["2-3 specific hedging strategies"]
  },
  "generatedAt": "${new Date().toISOString()}"
}

Important: Return valid JSON only. Amounts as numbers. Write all user-facing text (summary, explanation, vulnerabilities, hedgingSuggestions) naturally in ${getCurrentDisplayLanguage() === 'ja' ? 'Japanese' : 'English'}. No explanatory text before or after the JSON.
`;
...
중요: 유효한 JSON만 반환. 금액은 숫자(KRW). 한국어로 자연스럽게 작성한다. 실제 세법 기반 정확한 계산.
` : `
...
[Analysis Items]
1. Tax summary (estimated capital gains tax, income tax, effective tax rate)
2. Tax-saving strategies (prioritized, with estimated savings amounts)
3. Per-ticker sell timing recommendation (SELL_NOW / HOLD_FOR_TAX / TAX_LOSS_HARVEST)
4. Quarterly annual plan

${input.residency === 'KR' ?
  '[Korean Tax Law]\n- Overseas stock capital gains tax: 22% after ₩2.5M annual deduction\n- Comprehensive financial income tax: applies above ₩20M\n- Consider ISA / pension savings accounts' :
  '[US Tax Law]\n- Long-term vs Short-term Capital Gains\n- Tax-Loss Harvesting\n- Beware of Wash Sale Rule'}

[Output Format] Return only the JSON structure below:
{
  "residency": "${input.residency}",
  "taxSummary": {
    "estimatedCapitalGainsTax": <amount>,
    "estimatedIncomeTax": <amount>,
    "totalTaxBurden": <total amount>,
    "effectiveTaxRate": 12.5
  },
  "strategies": [
    {
      ...
      "potentialSaving": <tax saving amount>,
      "priority": "HIGH",
      "actionItems": ["Specific action item 1", "..."]
    }
  ],
  "sellTimeline": [
    {
      ...
      "name": "Apple",
      "suggestedAction": "HOLD_FOR_TAX",
      "reason": "3 months remaining until long-term capital gains rate applies",
      "optimalTiming": "After May 2026"
    }
  ],
  "annualPlan": [
    {"quarter": "Q1 2026", "actions": ["Open ISA account", "..."]},
    {"quarter": "Q2 2026", "actions": ["..."]},
    {"quarter": "Q3 2026", "actions": ["..."]},
    {"quarter": "Q4 2026", "actions": ["Year-end tax-loss harvesting"]}
  ],
  "generatedAt": "${new Date().toISOString()}"
}

Important: Return valid JSON only. Amounts as numbers (KRW). Write naturally in English. Base calculations on actual tax law.
`;
