...
중요: 유효한 JSON만 반환. 금액은 숫자로. ${getPromptLanguageInstruction()} 설명 텍스트를 JSON 앞뒤에 붙이지 마세요.
`;
...
중요: 유효한 JSON만 반환. 금액은 숫자(KRW). ${getPromptLanguageInstruction()} 실제 세법 기반 정확한 계산.
`;
...
[응답 규칙]
1. ${getPromptLanguageInstruction()}
